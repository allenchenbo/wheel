...
var wheel = {
  ...
  spin: function () {
    ...
    if (wheel.timerHandle == 0) {
      ...
      wheel.maxSpeed =
        Math.PI / (Math.floor(wheel.segments.length / 3) + Math.random()); // Randomly vary how hard the spin is
      wheel.frames = 0;
  ...
  onTimerTick: function () {
    ...
    if (finished) {
      ...
      if (wheel.recordMap[wheel.currentSegments]) {
        //統計數量
        wheel.recordMap[wheel.currentSegments] =
...
window.onload = function () {
  document
    .querySelector(".gold-button")
    .addEventListener("click", () => setWheel("gold"), false);
  document
    .querySelector(".red-button")
    .addEventListener("click", () => setWheel("red"), false);
  document
    .querySelector(".blue-button")
    .addEventListener("click", () => setWheel("blue"), false);
};
